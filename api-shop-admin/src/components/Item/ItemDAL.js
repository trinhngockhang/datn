...
export const createItem = async(item, shop_id) => {
    ...
    try {
        ...
        if(item.varians && item.varians.length > 0){
            ...
            const res = await dbUtil.execute(itemModelSql, [id, item.id, item.inventory, item.defaultPrice, itemRes]);
...
const createItemTable = async (item, shop_id) => {
    const id = uuidv4();
    const sql = 'INSERT INTO item(id, name, shop_id, sku, default_image, category_id, sub_category_id) VALUES(?, ?,?,?, ?, ?, ?)';
    const defaultImg = item.images && item.images.length > 0? item.images[0] : null;
    const itemRes = await dbUtil.execute(sql, [id, item.name, shop_id, item.id, defaultImg, item.category, item.sub_category_id]);
    console.log(itemRes);
    return id;
}
