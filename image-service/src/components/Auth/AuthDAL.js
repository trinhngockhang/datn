...
export const signUp = async ({ username, passwordHash, name, email, phone }) => {
  ...
  const sql = 'INSERT INTO shop(username, password, name, email, phone) VALUES (?, ?, ?, ?, ?)';
  await dbUtil.query(sql, [username, passwordHash, name, email, phone]);
};

export const checkUserExist = async (username) => {
  const sql = 'SELECT username FROM shop WHERE username = ?';
...
export const getUserById = async (userId) => {
  const sql = 'SELECT username, name, createdAt FROM shop WHERE id = ?';
