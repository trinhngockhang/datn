...
export const signUp = async (req, res) => {
  ...
  const { username, password, name, confirm, mobile, email, prefix } = req.body.user;
  if (password !== confirm) {
    res.status(400).send("wrong repassword");
    return;
  }
  const passwordHash = hash(password);
  await dbAccess.signUp({ username, passwordHash, name, email, phone: prefix + '' + mobile });
  res.ok();
};
